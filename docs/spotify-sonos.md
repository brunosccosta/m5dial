...
- **Controls shift between entities depending on where playback is.** When on iPhone: control via `media_player.spotify`. When on Sonos: control via `media_player.sala` (Spotify entity loses most features).
...
| Action | When on iPhone | When on Sala |
|---|---|---|
...
| Cast to iPhone | `media_player.select_source` → spotify, `source: "iPhone"` | — |
