...
## LVGL Montserrat font only covers ASCII 0x20–0x7E — use custom Latin fonts

The built-in `lv_font_montserrat_*` fonts only cover ASCII. Characters outside this range render as squares.

**Fix**: custom `font_montserrat_lat_14/24/28/48` fonts in `src/ui/fonts/` cover ASCII + Latin-1 Supplement (`0xA0–0xFF`), including degree symbol `°` (U+00B0) and all Portuguese accented chars (ã á â à ç é ê í ó ô õ ú etc.). Generated by `tools/gen_fonts.sh`. Range must start at `0xA0`, not `0xC0` — degree symbol is at U+00B0 which falls in `0xA0–0xBF`.

Use these instead of `lv_font_montserrat_*` everywhere **except** where LVGL built-in symbol glyphs are needed (`LV_SYMBOL_OK`, `LV_SYMBOL_CLOSE`, `LV_SYMBOL_WARNING` etc.) — those use codepoints in LVGL's Private Use Area only present in the built-in fonts. Affected files: `ErrorOverlay.cpp`, `ConfirmScreen.cpp` — keep using `lv_font_montserrat_*` there.

`CardLayout::fontHero/fontTitle/fontValue/fontDetail()` return the Latin fonts. All other `.cpp` files use `font_montserrat_lat_*` directly.

Still unsupported: em dash `—` (U+2014), euro `€` (U+20AC) — outside 0xC0–0xFF. Use ASCII substitutes (`-`, `EUR`).

`font_montserrat_cyr_24` covers ASCII + Latin Extended + Cyrillic — used by `LoveCard` for Russian + Portuguese messages.
